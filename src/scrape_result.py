...
PayTable01の列構造: [馬番/組合せ(td[0])] [払戻金(td[1])] [人気(td[2])]
払戻は td[1] を使う（td[-1] は人気列なので誤り）
"""
import time
import logging
import requests
...
def _parse_yen(td) -> int | None:
    ...
    txt = (td.get_text(strip=True)
             .replace(',', '').replace('円', '').replace('\xa0', '').strip())
    try:
        v = int(txt)
        return v if v >= 100 else None  # 100円未満は馬番や人気等なので除外
    except ValueError:
        return None
...
def fetch_race_result(race_id: str) -> dict:
    ...
    # ── 着順テーブル ──────────────────────────────────────────────────────
    # 列: 着順(0) | 枠(1) | 馬番(2) | 馬名(3) | ...（計15列）
    # 上位3行が1〜3着。[1:4] で確実に取得する。
    result_table = soup.find('table', class_='RaceTable01')
    if result_table:
        for tr in result_table.find_all('tr')[1:4]:
            tds = tr.find_all('td')
            if len(tds) < 4:
                continue
            try:
    ...
    # ── 払戻テーブル ──────────────────────────────────────────────────────
    # 列構造: [馬番/組合せ(td[0])] [払戻(td[1])] [人気(td[2])]
    # 複勝は th rowspan=3 で3行に分かれるため current_type で追跡する
