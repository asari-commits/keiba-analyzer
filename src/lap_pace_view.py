...
def _norm_rn(s):
    """レース名を正規化して、ラップCSVの短縮表記(例 'TVh賞・3勝'/'UHB賞Ｈ'/'キーンラ')と
    pred_dfの正式名の差を吸収する。
    例: '関屋記念(GⅢ)サラ系３歳以上' / 'TVh賞・3勝' → '関屋記念' / 'TVh賞' 。"""
    s = unicodedata.normalize("NFKC", str(s))
    s = re.sub(r"第\d+回", "", s)
    s = re.sub(r"[（(][^）)]*[）)]", "", s)            # (...)（グレード/条件）を全除去
    s = re.sub(r"[・･].*$", "", s)                     # 中黒以降(・3勝/・牝 等の付加情報)
    s = re.sub(r"(HG[123]|JG[123]|G[123]|GI{1,3}|OP|L|H)+$", "", s)
    s = re.sub(r"\d{3,4}$", "", s)                     # 末尾の距離(1000/1600等)
    s = s.replace("*", "").replace("＊", "")
    s = re.sub(r"[\s　]+", "", s)
    return s.strip()
