...
def add_pace_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # レース実ペース予想 × 馬のペース適性フィット（ご要望: コースでなくレース自体のペースで判定）
    #   出走馬の脚質構成（先行馬の比率）から当日の想定ペースを推定（結果PCIは使わない＝leakなし）。
    #   先行馬(style_avg3<0.35)が多い → ハイペース想定 → 持続力型(horse_lopci_fuku)有利
    #   少ない                       → スローペース想定 → 瞬発力型(horse_hipci_fuku)有利
    if 'style_avg3' in out.columns and all(c in out.columns for c in ('日付', '開催', 'Ｒ')):
...
META_COLS = ['日付_dt', '日付', '開催', 'Ｒ', '馬番', 'レース名', '馬名', '騎手', '着順_num', '人気',
             '単勝配当', '複勝配当',           # 回収率検証・集計用（モデル入力ではない結果列）
             '単勝オッズ_live', '人気_live',   # 出馬表CSV由来のオッズ/人気を予測後まで通す（表示・EV用）
             'doaku_score', 'horse_wet_n']   # 道悪適性タグ用（モデル特徴量ではない・表示通過列）
# ↑ レース名: 予測後まで通す（CSVのレース名を表示に使う。無いとnetkeiba補完に依存し、
#   延期などで日付がズレるとレース名が誤取得になるため）。モデル入力ではない。
