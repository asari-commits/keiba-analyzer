...
def suggest_from_memo(memo_text: str, eval_options: list, all_tags: list) -> dict:
    ...
    system = (
        ...
        "# 分類ルール（重要）\n"
        "・「不利」系タグ（出遅れ/包まれる/進路なし/前が壁/挟まれる/接触/外々を回された 等）は、"
        "他馬との接触や進路を塞がれた等『物理的な不利』が明確に書かれている場合のみ選ぶ。\n"
        "・ペースや位置取りが敗因（例: 前傾ラップで早めに仕掛けた/前が残った/後方から届かない）は"
        "『展開』系タグ（展開不利(前残り) 等）を選び、物理的な不利タグは付けない。\n"
        "・メモに書かれていないことは推測でタグ付けしない。該当しないタグは付けない（過剰付与しない）。\n"
        "・確信が持てないタグは省く。"
    )
    user = (
        f"# 観察メモ\n{memo_text}\n\n"
