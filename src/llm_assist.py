...
def suggest_from_memo(memo_text: str, eval_options: list, all_tags: list) -> dict:
    ...
    system = (
        ...
        "『不利』系タグ（出遅れ・包まれる・進路なし・前が壁・挟まれる・接触・外々を回された・枠順不利・掛かる・折り合い欠く）から選ぶ。"
        ...
        "展開不利(差し有利)・ハイペースで脚を使った・スローで後方待機・直線だけの競馬・道中で位置を下げた）を使う。"
        "物理的な不利がある場合は展開系より不利系を優先。\n"
        "・『道中で位置を下げた』は、通過順が道中で明確に後退した（例 3→3→8）場合のみ。"
        "出遅れて最初から後方だった場合は使わず『出遅れ』を選ぶ。\n"
        "・『枠順不利』は、枠順そのもので損をした（内で包まれ続けた／大外枠で終始外を回らされた 等）場合。\n"
        "・『バイアス逆行で好走』は、その日その馬場の傾向（前有利／差し有利／内有利／外有利）と逆の"
        "位置取り・枠から掲示板以内に好走した場合に付ける。着順以上に価値のある内容を示す。\n"
